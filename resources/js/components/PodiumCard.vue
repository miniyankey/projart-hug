...
const rankLabel = computed(() => ({ 1: '1er', 2: '2ème', 3: '3ème' }[props.rank]))
...
const rankPadding = computed(() => ({ 1: 'py-14', 2: 'py-9', 3: 'py-5' }[props.rank]))
</script>
...
<template>
    <div class="flex flex-col items-center">
        ...
        <div class="mb-3 flex h-14 items-end justify-center">
            <img :src="logoSrc" :alt="logoAlt" class="max-h-14 max-w-[150px] object-contain" />
        </div>

        <!-- Carte -->
        <div class="relative w-full overflow-hidden border-[3px]" :style="cardStyle">
            <!-- Rivets haut -->
            ...
            <div class="relative flex items-center justify-center" :class="rankPadding">
                <div class="absolute bottom-2 left-2 h-3 w-3" :style="rivetStyle" />
            ...
            <div class="bg-[#1b2735] px-4 py-4 font-mono text-white">
                <p class="mb-2 text-center text-xs font-bold tracking-wider">{{ category }}</p>
                <p class="text-center text-xs leading-relaxed text-gray-300">{{ description }}</p>
            </div>
        </div>
    </div>
</template>
