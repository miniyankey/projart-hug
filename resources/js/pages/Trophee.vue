...
const historyMilestones = [
    { year: '2008', tag: t('trophee.histoire.tag_2008'), color: '#B2D4A8', shadow: '#4E8A42' },
    { year: '2010', tag: t('trophee.histoire.tag_2010'), color: '#D4A0A0', shadow: '#A05050' },
    { year: '2026', tag: t('trophee.histoire.tag_2026'), color: '#B5CEED', shadow: '#4A7AAD' },
];
...
onMounted(() => {
    ...
    ctx = gsap.context(() => {
        ...
        gsap.timeline({ defaults: { ease: 'power3.out' } })
            ...
            .from('.anim-hero-subtitle', { opacity: 0, y: 25, duration: 0.7 }, '-=0.5')
            .from('.anim-hero-ctas', { opacity: 0, y: 20, duration: 0.6 }, '-=0.35');

        // Mascotte — entrée depuis la droite + flottement infini
        gsap.timeline()
            .from('.anim-hero-mascot', {
                x: 80, opacity: 0, rotation: -8, duration: 1, ease: 'back.out(1.4)', delay: 0.2,
            })
            .to('.anim-hero-mascot', {
                y: -16, rotation: 2, duration: 2.4, repeat: -1, yoyo: true, ease: 'sine.inOut',
            }, '+=0.1');

        // Bulle — pop in puis typewriter
        gsap.from('.anim-speech-bubble', {
            opacity: 0, scale: 0.5, duration: 0.5, delay: 1.1, ease: 'back.out(2.5)',
        });
        gsap.to(charObj, {
            ...
            delay: 1.4,
            onUpdate() {
                displayedBubbleText.value = bubbleFullText.slice(0, Math.round(charObj.n));
            },
        });
        ...
        gsap.timeline({
            scrollTrigger: { trigger: '.anim-trophy-section', start: 'top 70%' },
            defaults: { ease: 'power3.out' },
        })
            .from('.anim-trophy-text > *', {
                opacity: 0, x: -50, stagger: 0.18, duration: 0.75,
            });

        // Trophée : rideau de révélation
        gsap.timeline({
            scrollTrigger: { trigger: '.anim-trophy-section', start: 'top 65%' },
        })
            .to('.anim-curtain-left', { x: '-101%', duration: 1.3, ease: 'power3.inOut', delay: 0.4 })
            .to('.anim-curtain-right', { x: '101%', duration: 1.3, ease: 'power3.inOut' }, '<');
        ...
        gsap.timeline({
            ...
            .from('.anim-podium-3', { y: 120, opacity: 0, duration: 1.1 }, '-=0.2')
            .from('.anim-podium-2', { y: 120, opacity: 0, duration: 1.1 }, '-=0.6')
            .from('.anim-podium-1', { y: 120, opacity: 0, duration: 1.1 }, '-=0.6');

        // ── Histoire — séquentiel ─────────────────────────────
        gsap.timeline({
            scrollTrigger: { trigger: '.anim-histoire-title', start: 'top 85%' },
            defaults: { ease: 'power2.out' },
        })
            .from('.anim-histoire-title', { opacity: 0, y: 30, duration: 0.7 });

        gsap.from('.anim-milestone', {
            opacity: 0, x: -120, stagger: 0.35, duration: 0.8, ease: 'power3.out',
            scrollTrigger: { trigger: '.anim-milestone', start: 'top 90%' },
        });

        // ── Stats — compteur animé ────────────────────────────
        gsap.from('.anim-stat', {
            opacity: 0, y: 40, stagger: 0.18, duration: 0.7, ease: 'power2.out',
        ...
        statConfigs.forEach((stat, i) => {
            ...
            gsap.to(counter, {
                ...
                scrollTrigger: { trigger: '.anim-stats-section', start: 'top 80%' },
                onUpdate() {
                    statDisplayValues.value[i] = `${Math.round(counter.n)}${stat.suffix}`;
                },
            });
        });
...
<template>
    <PublicLayout>
        ...
        <div ref="root">
            ...
            <section style="background-color: #EDE9F8" class="py-24 lg:py-36">
                <div class="mx-auto max-w-7xl px-6">
                    <div class="grid items-center gap-10 lg:grid-cols-2">
                        <div>
                            <h1 class="anim-hero-title font-pixel text-[1.35rem] leading-loose text-gray-900">
                                {{ t('trophee.hero.title') }}
                            </h1>
                            <p class="anim-hero-subtitle mt-6 max-w-md leading-relaxed text-gray-700">
            ...
            <!-- Trophée visuel -->
            <section class="anim-trophy-section py-32" style="background-color: #3d8080;">
                <div class="mx-auto max-w-7xl px-6">
                    <div class="grid items-center gap-12 lg:grid-cols-2">
                        <div class="anim-trophy-text">
                            <span class="font-pixel inline-block bg-yellow-300 px-3 py-1 text-[0.6rem] tracking-widest text-black uppercase">
                                Remis chaque année
                            </span>
                            <h2 class="font-pixel mt-6 text-[1.5rem] leading-loose text-white">
                                {{ t('trophee.trophee_visuel.title') }}
                            </h2>
                            <div class="mt-4 h-1 w-12 bg-teal-200"></div>
                            <p class="mt-6 max-w-md text-base leading-relaxed text-teal-100">
                                {{ t('trophee.trophee_visuel.desc') }}
                            </p>
                        </div>
                        <div class="flex items-center justify-center lg:justify-end">
                            <div class="relative overflow-hidden rounded-3xl" style="width: 380px; height: 520px;">
                                <!-- Halo -->
                                <div
                                ...
                                />
                                <!-- Rideaux -->
                                <div class="anim-curtain-left pointer-events-none absolute inset-y-0 left-0 z-20 w-1/2 border-r-2 border-teal-900" style="background-color: #3d8080;"></div>
                                <div class="anim-curtain-right pointer-events-none absolute inset-y-0 right-0 z-20 w-1/2 border-l-2 border-teal-900" style="background-color: #3d8080;"></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Vainqueurs -->
            <section id="vainqueurs" style="background-color: #EDE9F8;" class="py-32">
                <div class="anim-vainqueurs mx-auto max-w-7xl bg-white px-20 py-12 shadow-[8px_8px_0px_0px_#3d8080]">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex flex-1 flex-col items-center gap-4">
                            <div class="font-pixel inline-block border-4 border-teal-800 bg-teal-600 px-6 py-2 text-lg text-white shadow-[4px_4px_0px_0px_#2a5f5f]">
                                {{ t('trophee.vainqueurs.edition') }} {{ currentEdition.year }}
                            </div>
                            <p v-if="isSuspended" class="text-sm italic text-gray-400">
                                {{ t('trophee.vainqueurs.suspended') }}
                            </p>
                            <div class="flex w-full items-end justify-center gap-3">
                                <PodiumCard
                                    :rank="2"
                                    :logo-src="currentEdition.winners.find(w => w.rank === 2).logo"
                                    :logo-alt="currentEdition.winners.find(w => w.rank === 2).name"
                                    :category="currentEdition.winners.find(w => w.rank === 2).name"
                                    description=""
                                    class="anim-podium-2 hidden w-40 lg:flex"
                                />
                                <PodiumCard
                                    :rank="1"
                                    :logo-src="currentEdition.winners.find(w => w.rank === 1).logo"
                                    :logo-alt="currentEdition.winners.find(w => w.rank === 1).name"
                                    :category="currentEdition.winners.find(w => w.rank === 1).name"
                                    description=""
                                    class="anim-podium-1 w-44"
                                />
                                <PodiumCard
                                    :rank="3"
                                    :logo-src="currentEdition.winners.find(w => w.rank === 3).logo"
                                    :logo-alt="currentEdition.winners.find(w => w.rank === 3).name"
                                    :category="currentEdition.winners.find(w => w.rank === 3).name"
                                    description=""
                    ...
                    <div class="mt-8 flex justify-center gap-2">
                        <button
                            ...
                            :class="i === currentIndex ? 'w-6 bg-teal-600' : 'w-2.5 bg-teal-200'"
            ...
            <!-- Histoire -->
            <section class="anim-histoire-section py-32" style="background-color: #EDE9F8;">
                <div class="mx-auto max-w-7xl px-6">
                    <h2 class="anim-histoire-title font-pixel mb-16 text-center text-[1.1rem] leading-loose text-gray-900">
                        {{ t('trophee.histoire.title') }}
                    </h2>

                    <div class="relative">
                        <!-- Ligne horizontale -->
                        <div class="absolute top-6 right-0 left-0 h-1 bg-gray-900"></div>
                        ...
                        <div class="grid grid-cols-3 gap-8">
                            ...
                            >
                                ...
                                <div
                                    ...
                                    :style="{ backgroundColor: m.color, boxShadow: `3px 3px 0 ${m.shadow}` }"
                                ></div>

                                <!-- Tag -->
                                <span
                                    class="font-pixel mb-2 px-3 py-1 text-xs text-white"
                                ...
                                <!-- Année -->
                                <span class="font-pixel mb-6 text-3xl text-gray-900">{{ m.year }}</span>

                                <!-- Carte description -->
                                <div
                                    class="w-full bg-white p-5"
                                    :style="{ boxShadow: `4px 4px 0 ${m.shadow}` }"
                                >
            ...
            <section
                ...
                style="background-color: #2a5f5f; background-image: radial-gradient(circle, rgba(255,255,255,0.07) 1px, transparent 1px); background-size: 28px 28px;"
            >
                <div class="mx-auto max-w-7xl px-6">
